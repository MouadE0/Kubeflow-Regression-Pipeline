...
def elastic_net_regression(split_data: dict, score: float) -> float:
    ...
    # score = mean_squared_error(y_test, x_test_poly)
    y_pred = regr.predict(x_test_small)
    score = mean_squared_error(y_test_small, y_pred)

    print(f"The Score of Elastic Net Regression is {score}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Elastic Net Regression Modeling')
    ...
    elastic_net_regression(split_data=args.split_data, score=args.score)
