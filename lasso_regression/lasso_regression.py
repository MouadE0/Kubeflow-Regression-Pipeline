...
def lasso_regression(clean_data: dict, score: float) -> float:

    prefix = 'gs://data-cs-kubeflow'
    
    # read data from args.preprocess
    # train_data = pa.read_parquet(clean_data['train_data'])
    # test_data = pa.read_parquet(clean_data['test_data'])    
    ...
    score = mean_squared_error(y_test_small, regr.predict(x_test_small))
    print(f"The Score of Lasso Regression is {score}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Lasso Regression Modeling')
    parser.add_argument('--score', type=str)
    parser.add_argument('--clean_data', type=dict)
    args = parser.parse_args()

    # Creating the directory where the output file will be created (the directory may or may not exist).
    Path(args.score).parent.mkdir(parents=True, exist_ok=True)
    
    lasso_regression(clean_data=args.clean_data, score=args.score)
